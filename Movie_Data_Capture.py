...
from plugins.mdc_self.ADC_function import file_modification_days, get_html, parallel_download_files
from plugins.mdc_self.number_parser import get_number
from plugins.mdc_self.core import core_main, core_main_no_net_op, moveFailedFolder
...
def start():
    ...
    if 再运行延迟 > 0 and conf.stop_counter() > 0:
        while True:
            try:
                logfile = main(args)
                (扫描电影数, 已处理, 完成数) = 分析结果元组 = tuple(分析日志文件(
                ...
                if all(isinstance(v, int) for v in 分析结果元组):
                    ...
                    log_temp = f'All movies:{扫描电影数}  processed:{已处理}  successes:{完成数}  remain:{剩余个数}' + \
                               '  Elapsed time {}'.format(
                    ...
                    next_log = f'Next run time: {下次运行.strftime("%H:%M:%S")}, rerun_delay={再运行延迟}, press Ctrl+C stop run.'
                    print(next_log)
                    _LOGGER.info(next_log)
                    time.sleep(再运行延迟)
                else:
                    break
            except:
